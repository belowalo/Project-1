...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    """
    # Initialize billing date based on the first event's timestamp
    ...
    for event in log['events']:
        ...
        event_month, event_year = event_time.month, event_time.year

        # If event belongs to a new month, advance all customers
        if event_month != billing_date.month or event_year != billing_date.year:
            new_month(customer_list, event_month, event_year)
            billing_date = event_time

        # Process call event
        if event['type'] == 'call':
            call = Call(event['src_number'], event['dst_number'], event_time,
                        event['duration'], tuple(event['src_loc']), tuple(event['dst_loc']))
            caller = find_customer_by_number(event['src_number'], customer_list)
            picker = find_customer_by_number(event['dst_number'], customer_list)
            if caller is not None and picker is not None:
                caller.make_call(call)
                picker.receive_call(call)
            else:
                print("ERROR: Caller or picker not found.")
