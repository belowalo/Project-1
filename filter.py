...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if filter_string.isdigit():
            ...
            matching_customer = next((cust for cust in customers if cust.get_id() == customer_id), None)
            if matching_customer:
                history_calls = matching_customer.get_history()
                return [call for call in data if call in history_calls[0] or call in history_calls[1]]
        return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        specified in the handout.
        """
        if len(filter_string) > 0 and filter_string[0].isalpha() and filter_string[1:].isdigit():
            threshold = int(filter_string[1:])
            if filter_string[0].lower() == 'l':
                return [call for call in data if call.duration < threshold]
